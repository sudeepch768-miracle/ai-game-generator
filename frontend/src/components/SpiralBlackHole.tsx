...
export const SpiralBlackHole: React.FC<SpiralBlackHoleProps> = ({
  ...
  useEffect(() => {
    ...
    // Load current black hole cutout (100% transparent, all background blackspace removed)
    const baseImg = new Image();
    baseImg.src = '/assets/blackhole_cutout.png';
    let imgLoaded = false;
    baseImg.onload = () => {
      imgLoaded = true;
    };
    ...
    const render = () => {
      ...
      const displayHeight = canvas.clientHeight || Math.round(width * 0.68);
      ...
      const s = displayWidth / 440; // Proportional scale factor relative to 440x440 cutout
      ...
      // 1. Draw current black hole icon (blackhole_cutout.png: transparent, no blackspace)
      if (imgLoaded) {
        ctx.save();
        ctx.translate(cx, cy);
        ctx.scale(s, s);
        ...
        // Draw centered on void (440x440 cutout)
        ctx.drawImage(baseImg, -220, -220, 440, 440);
        ctx.restore();
      }
      ...
      if (hacker) {
        hazeGrad.addColorStop(0, 'rgba(0, 255, 102, 0.38)');
        hazeGrad.addColorStop(0.35, 'rgba(0, 242, 254, 0.18)');
        hazeGrad.addColorStop(0.7, 'rgba(0, 50, 20, 0.08)');
        hazeGrad.addColorStop(1, 'rgba(0, 0, 0, 0)');
      } else {
        hazeGrad.addColorStop(0, 'rgba(147, 51, 234, 0.42)');
        hazeGrad.addColorStop(0.35, 'rgba(192, 132, 252, 0.2)');
        hazeGrad.addColorStop(0.7, 'rgba(75, 20, 120, 0.08)');
        hazeGrad.addColorStop(1, 'rgba(0, 0, 0, 0)');
      }
      ...
      // Flowing logarithmic spiral arms (Keplerian differential rotation: v ~ 1 / r^0.65)
